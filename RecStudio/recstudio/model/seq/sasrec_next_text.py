...
class SASRecItemEncoder(torch.nn.Module):

    def __init__(self, config, train_data, id_embed_dim) -> None:
        super().__init__()
        ...
        if self.config['model']['item_layer_norm']:
    ...
    def forward(self, item_ids):
        item_id_embeddings = self.item_emb(item_ids) # [B, D] or [B, L, D]
        item_title_embeddings = self.item_title_emb(item_ids) # [B, D2] or [B, L, D2]
        
        if self.config['model']['text_id_concat']:
            return self.item_mlp(torch.cat([item_id_embeddings, item_title_embeddings], dim=-1))
        else:
            if self.config['model']['item_layer_norm']:
                return self.item_layernorm(self.item_mlp(item_title_embeddings)) + item_id_embeddings
            else:
                return self.item_mlp(item_title_embeddings) + item_id_embeddings
...
class SASRec_Next_Text(basemodel.BaseRetriever):
    r"""
    ...
    def _init_model(self, train_data, drop_unused_field=True):
        self.id_embed_dim = self.config['model']['id_embed_dim']
    ...
    def _init_parameter(self):
        ...
        self.item_encoder.item_title_vectors = torch.from_numpy(np.load(self.config['model']['item_title_vectors'])).type(torch.float)
        self.item_encoder.item_title_emb = torch.nn.Embedding.from_pretrained(self.item_encoder.item_title_vectors, freeze=True, padding_idx=0)
    ...
    def _get_item_encoder(self, train_data) -> SASRecItemEncoder:
        return SASRecItemEncoder(self.config, train_data, self.id_embed_dim)
    ...
    def _get_item_vector(self):
        item_ids = torch.arange(1, self.num_items, device=self._parameter_device)
