...
class TripletDataset(Dataset):
    ...
    def _fill_nan(self, feat, mapped=False):
        ...
        def is_nan(x):
            '''
            check whether x is None, return a bool value. 
            x can be a np.array. 
            '''
            if x is None:
                return True 
        ...
        for field in feat:
            ...
            if ftype == 'float':
                ...
                feat[field] = \
                    feat[field].map(lambda x: np.array([], dtype=dtype) if is_nan(x) else x)
            elif ftype == 'float_seq':
                feat[field] = \
                    feat[field].map(lambda x: np.array([], dtype=np.float64) if is_nan(x) else x)
            elif ftype == 'str':
    ...
    def _load_feat(self, feat_path, header, sep, feat_cols, update_dict=True):
        ...
        for i, (col, t) in enumerate(zip(fields, types_of_fields)):
            if not t.endswith('seq'):
                ...
                continue
            # we need to get the max length of the seq, so None in the seq should be removed.
